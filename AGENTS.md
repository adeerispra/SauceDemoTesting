...
When asked to **"test this"** or **"run this feature"**, always complete all steps below in order:

1. **Read relevant context files** — read `rules/saucedemo-context.mdc` and the relevant `test-context/flows/` files
2. **Generate test cases** — create `tc_template/{TICKET_ID}_Test_Cases.xlsx` following `rules/test-case-generation.mdc`
3. **Execute each test case** — follow `rules/browser-automation.md` for session, recording, and startup order
4. **Document results** — save recordings to `Testing Result/` following the naming convention
...
| File | Purpose |
|---|---|
| `rules/saucedemo-context.mdc` | Platform overview, environments, credentials, known issues |
| `rules/test-case-generation.mdc` | Excel format and workflow for generating test cases |
