...
If `JIRA_BASE_URL`, `JIRA_EMAIL`, or `JIRA_API_TOKEN` is empty after sourcing `.env`, stop and ask the user to fill in `.env` before proceeding.
...
The `content_url` will be in the format: `$JIRA_BASE_URL/rest/api/3/attachment/content/{id}`
